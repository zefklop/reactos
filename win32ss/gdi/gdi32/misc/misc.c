...
GdiValidateHandle(HGDIOBJ hobj)
{
    ...
    Handle.AsHandle = hobj;

    if ((Entry->DetailedType.BaseType != 0) &&
        (Entry->Unique.ObjectType == Handle.ObjectType))
    {
...
BOOL GdiGetHandleUserData(HGDIOBJ hGdiObj, DWORD ObjectType, PVOID *UserData)
{
    ...
    Handle.AsHandle = hGdiObj;
    Template.AsULong = ObjectType;

    /* Check if we have the correct type */
    if ((Handle.ObjectType != Template.ObjectType) ||
        (Entry->Unique.ObjectType != Template.ObjectType) ||
        (Entry->DetailedType.BaseType != Template.DetailedType.BaseType))
    {
        return FALSE;
    }
