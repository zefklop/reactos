                   ...
                   IN PRTL_HEAP_PARAMETERS Parameters)
{
    ...
    DeCommitFreeBlockThreshold = ROUND_UP(Parameters->DeCommitFreeBlockThreshold, PAGE_SIZE);
    if (DeCommitFreeBlockThreshold > (PAGE_SIZE * 4))
        DeCommitFreeBlockThreshold = (PAGE_SIZE * 4);
    DeCommitFreeBlockThreshold >>= HEAP_ENTRY_SHIFT;
    ...
    HeaderSize = FIELD_OFFSET(HEAP, FreeHints[DeCommitFreeBlockThreshold])
                    + (ROUND_UP(DeCommitFreeBlockThreshold, 32) / 32) * sizeof(ULONG);
                      ...
                      BOOLEAN Remove)
{
    ...
    if (Remove)
        RtlpRemoveFreeBlock(Heap, FreeEntry, TRUE);
