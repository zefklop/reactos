                      ...
                      BOOLEAN Remove)
{
    ...
    if (Remove)
        RtlpRemoveFreeBlock(Heap, FreeEntry, FALSE, TRUE);
...
)
{
    ...
    else
    {
        ...
        {
            /* Decommit this block */
            RtlpDeCommitFreeBlock(Heap, (PHEAP_FREE_ENTRY)HeapEntry, BlockSize, FALSE);
        }
        else
        {
        ...
        /* Check if we decommitted enough */
        if ((Heap->TotalFreeSize >= Heap->DeCommitTotalFreeThreshold)
            && (Heap->CommitRoutine == NULL))
        {
            PLIST_ENTRY ListEntry = Heap->FreeLists[0].Blink;

            while (ListEntry != &Heap->FreeLists[0])
            {
                PHEAP_FREE_ENTRY ToDecommit = CONTAINING_RECORD(ListEntry, HEAP_FREE_ENTRY, FreeList);
                ListEntry = ListEntry->Blink;

                if (RtlpDeCommitFreeBlock(Heap, ToDecommit, ToDecommit->Size, TRUE))
                {
                    ListEntry = Heap->FreeLists[0].Blink;
                }
                      ...
                      IN SIZE_T Index)
{
    ...
    {
        PHEAP_FREE_ENTRY FreeEntry = (PHEAP_FREE_ENTRY)(InUseEntry + InUseEntry->Size);
